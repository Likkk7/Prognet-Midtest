...
@if ($value[0]=='textarea')
    <div class="mb-4">
        <label for="exampleFormControlInput1" class="form-label">{{ $value[1] }}</label>
        <textarea class="form-control @error($key) is-invalid @enderror" id="exampleFormControlTextarea1" rows="5" placeholder="{{ $value[1] }}" name="{{ $key }}"></textarea>
        @error($key)
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </div>
